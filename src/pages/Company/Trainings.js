...
const sanitizePincode = (input) => {
  ...
  const handleEditTrainer = (trainer) => {
    setTrainerForm({
      ...
      city: trainer.city || '',
      state: trainer.state || '',
       district: trainer.district || '', // ADD THIS LINE
...
const handleTrainerChange = (e) => {
  ...
  if (value !== trainerForm.state) {
    setTrainerForm((prev) => ({ ...prev, state: processedValue, district: '' }));
    return;
  }
  break;
  case 'district':
...
// तुमच्या existing Trainings component मध्ये हा function update करा:
...
const handleTrainerSubmit = async (e) => {
  ...
  try {
    if (editingTrainer) {
      ...
      // **UPDATED: NEW TRAINER REGISTRATION - NO FIREBASE AUTH**
      ...
      const newTrainerData = {
        ...
        active: true,
        role: 'trainer',
        firstTime: true, // Mark for first-time login
        lastLoginAt: null,
        authMethod: 'manual', // No Firebase Auth initially
        uid: null // Will be set when password is created
      };
      ...
      alert('Trainer registered successfully! They can now login with their phone number initially.');
    }
            ...
            <div style={{
              ...
              <div>
                ...
                >
                  ...
                  <select
                    name="district"
                    value={trainerForm.district}
                    onChange={handleTrainerChange}
                    required
                    disabled={!trainerForm.state}
                    style={{
                      width: '100%',
                      padding: '8px 12px',
                      border: '1px solid #d1d5db',
                      borderRadius: '6px',
                      fontSize: '14px',
                      backgroundColor: !trainerForm.state ? '#f9fafb' : 'white',
                      cursor: !trainerForm.state ? 'not-allowed' : 'pointer'
                    }}
                  >
                    <option value="">
                      {!trainerForm.state ? 'First select state' : 'Select District'}
                    </option>
                    {getDistrictsForState(trainerForm.state).map((district) => (
